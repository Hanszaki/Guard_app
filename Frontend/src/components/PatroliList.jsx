...
const PatroliList = () => {
  ...
  const getPatroli = async () => {
    try {
      ...
      let url = `http://localhost:5000/patroli?search_query=${encodedKeyword}&page=${page}&limit=${limit}&startDate=${startDate}&endDate=${endDate}`;
  ...
  const handleDeletePatroli = async (id) => {
    ...
    if (isConfirmed) {
      try {
        ...
        console.error("Error deleting patroli:", error.message);
      }
    }
  };
  const searchData = (e) => {
            ...
            <tbody>
              {patroli.map((patroli) => (
                <tr key={patroli.id}>
                  ...
                  <td>
                    <p className="title is-7">{patroli.urai_temuan}</p>
                  </td>
